...
def main():
    ...
    with open(out_file_name,"w") as fhandle:
        fhandle.write("Position\tGeneStart\tGeneEnd\tGeneName\tPositionInGene\tLocusTag\tPositionInGenome\tProduct\n")
                ...
                if True:
                    ...
                    for k in annot.features:
                        if k.type=="CDS":
                            ...
                            if (int(j)>=start) and (int(j)<=end):
                                ...
                                outp=str(j)+"\t"+str(start)+"\t"+str(end)+"\t"+str(gene)+"\t"+str(genePos)+"\t"+str(locus)+"\tCoding/genic\t"+str(product)
                                check=True 
                                break
                            else:
                                pass
                    if check:
                        fhandle.write(str(outp)+"\n")
                         ...
                         fhandle.write(str(j)+"\tN/A\tN/A\tN/A\tN/A\tN/A\tIntergenic region\tIntergenic region\n")
            status.next()
    status.finish()
